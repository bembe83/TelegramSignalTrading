...
from metatrader_json_executor import ExecutorError, execute_order
...
def load_chat_config(config_file):
    ...
    global MT5_EXECUTION_MODE, MT5_LOGIN, MT5_PASSWORD, MT5_SERVER
    global MT5_TERMINAL_PATH, MT5_MAGIC, MT5_DEVIATION
    ...
    try:
        ...
        MT5_EXECUTION_MODE = (section.get("MT5_EXECUTION_MODE", "file") or "file").strip().lower()
        if MT5_EXECUTION_MODE not in {"direct", "file"}:
            raise ValueError("MT5_EXECUTION_MODE must be 'direct' or 'file'.")

        MT5_LOGIN = int(section.get("MT5_LOGIN", "0") or "0")
        MT5_PASSWORD = section.get("MT5_PASSWORD", "")
        MT5_SERVER = section.get("MT5_SERVER", "")
        mt5_terminal_path_raw = section.get("MT5_TERMINAL_PATH", "")
        MT5_TERMINAL_PATH = mt5_terminal_path_raw.strip() or None
        MT5_MAGIC = int(section.get("MT5_MAGIC", "20260325") or "20260325")
        MT5_DEVIATION = int(section.get("MT5_DEVIATION", "20") or "20")
        ...
        print(f"MT5 execution mode: {MT5_EXECUTION_MODE}")
...
def save_message_to_db(msg_id, message_text, action=None, full_message=None, ticket=None):
    """Save a processed message to the database."""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute(
        "INSERT OR REPLACE INTO messages (msg_id, message_text, timestamp, action, ticket, source, full_message) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (msg_id, message_text, datetime.now(), action, ticket, DB_SOURCE, full_message),
    )
    conn.commit()
    conn.close()
...
def execute_trade(order_data, msg_id):
    """Execute order directly for MT5 in direct mode; otherwise generate JSON file."""
    # MT4 always remains file-based.
    if TRADING_PLATFORM != "MT5":
        generate_json_file(order_data, msg_id)
        return None

    if MT5_EXECUTION_MODE != "direct":
        generate_json_file(order_data, msg_id)
        return None

    if MT5_LOGIN <= 0 or not MT5_PASSWORD or not MT5_SERVER:
        raise ValueError(
            "MT5 direct mode requires MT5_LOGIN, MT5_PASSWORD, and MT5_SERVER in chat config."
        )
    ...
    print(f"Direct MT5 execution result: {result}")
    # MT5 direct responses can expose trade identifiers under different keys.
    ticket = result.get("order") or result.get("deal") or result.get("position")
...
async def process_message(client, msg_id):
    ...
    try:
        ...
        ticket = execute_trade(order_data, str(msg_id))
        save_message_to_db(str(msg_id), cleaned_text, order_data["action"], cleaned_text, ticket=ticket)
...
async def main():
    ...
    async def handler(event):
        ...
        ticket = execute_trade(order_data, msg_id)
        save_message_to_db(msg_id, cleaned_text, order_data["action"], cleaned_text, ticket=ticket)
